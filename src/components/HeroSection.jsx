...
const TastingNote = ({ title, description }) => {
  return (
    <div>
      <span className="block font-serif text-2xl text-coffee-primary mb-1">
        {title}
      </span>
      <span className="text-sm text-coffee-secondary font-sans">
        {description}
      </span>
    </div>
  );
...
const FloatingChip = ({ flavor, origin }) => {
  return (
    <div className="absolute -bottom-6 -left-6 bg-white p-6 editorial-shadow rounded-xl max-w-[200px] border border-coffee-secondary/10 z-20">
      <div className="flex gap-1 mb-3">
        <span className="w-4 h-4 rounded-full bg-coffee-accent" />
        <span className="w-4 h-4 rounded-full bg-coffee-primary opacity-20" />
      </div>
      <p className="font-serif text-lg text-coffee-primary leading-tight">
        {flavor}
      </p>
      <p className="text-xs text-coffee-secondary font-sans mt-1">{origin}</p>
    </div>
  );
};
  ...
  return (
    ...
    >
      ...
      <div className="w-full md:w-1/2 flex flex-col items-start z-10">
        {/* Season Badge */}
        <div className="mb-4 inline-flex items-center gap-2 px-4 py-1.5 bg-coffee-accent/15 rounded-full">
          <span className="text-[10px] font-bold uppercase tracking-widest text-coffee-primary">
            Season&apos;s Selection
          </span>
        </div>

        {/* Headline — satu h1 per halaman (a11y rule) */}
        <h1 className="font-serif text-5xl md:text-7xl lg:text-8xl text-coffee-primary leading-[1.1] mb-6 -tracking-[0.02em]">
          Seni di Setiap{" "}
          <span className="italic font-normal">Seduhan</span>
        </h1>

        {/* Subtitle */}
        <p className="font-sans text-lg md:text-xl text-coffee-secondary max-w-md mb-10 leading-relaxed">
        ...
        <div className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto">
          <button
            className="crema-gradient text-white px-8 py-4 rounded-xl font-sans text-sm font-semibold uppercase tracking-[0.05rem] hover:scale-[1.02] active:scale-[0.98] transition-all duration-300 editorial-shadow"
          ...
          <button
            className="text-coffee-primary px-8 py-4 font-sans text-sm font-semibold uppercase tracking-[0.05rem] border-b-2 border-transparent hover:border-coffee-accent transition-all duration-300 inline-flex items-center gap-2 group"
        ...
        {/* Tasting Notes Grid */}
        <div className="mt-16 grid grid-cols-1 sm:grid-cols-2 gap-8 border-l-2 border-coffee-secondary/20 pl-8">
          <TastingNote
            title="Single Origin"
      ...
      <div className="w-full md:w-1/2 relative flex justify-center items-center">
        ...
        <div
          className="absolute -right-20 top-0 w-full h-full bg-coffee-background rounded-full blur-[80px] opacity-60 pointer-events-none"
          aria-hidden="true"
        />
        ...
        <div className="relative w-full aspect-[4/5] md:aspect-square group">
          {/* Asymmetric Frame */}
          <div
            className="absolute inset-0 bg-coffee-secondary/10 translate-x-4 translate-y-4 rounded-2xl -rotate-1 transition-transform group-hover:rotate-0 duration-500"
            aria-hidden="true"
          />

          {/* Primary Hero Image */}
          <div className="relative w-full h-full rounded-2xl overflow-hidden editorial-shadow bg-coffee-secondary/5">
            <img
          ...
          <div className="hidden lg:block absolute -top-10 -right-10 w-40 h-40 rounded-full overflow-hidden border-8 border-coffee-background editorial-shadow z-20">
            <img
