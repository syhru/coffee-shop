...
const ValueChip = ({ label }) => {
  return (
    <span className="px-4 py-1.5 rounded-full bg-coffee-primary text-white text-[10px] font-bold uppercase tracking-widest">
      {label}
    </span>
  );
};
...
const PlanCard = ({ name, subtitle, price, features, badge, isHighlighted }) => {
  if (isHighlighted) {
    return (
      <article className="relative bg-coffee-primary text-white p-10 md:p-14 flex flex-col z-10 shadow-2xl md:scale-105 rounded-2xl">
        {/* Badge */}
        {badge && (
          <div className="absolute top-6 right-6">
            <span className="bg-coffee-accent text-coffee-primary px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider">
              {badge}
            </span>
          </div>
        )}
        ...
        <div className="mb-12">
          ...
          <p className="text-white/50 font-sans text-sm mb-8">{subtitle}</p>
        ...
        <ul className="space-y-6 mb-12 flex-grow">
          {features.map((feature, index) => (
            <li key={index} className="flex items-start gap-3">
              <span
                className="material-symbols-outlined text-coffee-accent text-sm mt-1"
                aria-hidden="true"
              >
                check_circle
              </span>
        ...
        <button
          className="w-full py-4 bg-coffee-accent text-coffee-primary rounded-xl text-xs font-bold uppercase tracking-wider hover:bg-coffee-accent/80 transition-colors duration-300"
  ...
  return (
    <article className="bg-coffee-background p-10 md:p-14 flex flex-col transition-all duration-300 hover:bg-coffee-secondary/5 rounded-2xl">
      <div className="mb-12">
        <h3 className="text-2xl font-serif text-coffee-primary mb-2">
          {name}
        </h3>
        <p className="text-coffee-secondary font-sans text-sm mb-8">
          {subtitle}
        </p>
        <div className="flex items-baseline gap-1">
          <span className="text-4xl font-serif font-bold text-coffee-primary">
            {price}
          </span>
          <span className="text-coffee-secondary text-sm">/ month</span>
        </div>
      </div>
      ...
      <ul className="space-y-6 mb-12 flex-grow">
        {features.map((feature, index) => (
          <li key={index} className="flex items-start gap-3">
            ...
            <span className="text-coffee-primary text-sm">{feature}</span>
          </li>
        ))}
      </ul>

      <button
        className="w-full py-4 bg-coffee-primary text-white rounded-xl text-xs font-bold uppercase tracking-wider hover:bg-coffee-secondary transition-all duration-300"
        aria-label={`Berlangganan paket ${name}`}
      >
        Subscribe Now
      </button>
    ...
    >
      ...
      <div className="relative h-[500px] md:h-[614px] flex items-center justify-center overflow-hidden">
        ...
        <div className="absolute inset-0 z-0">
          <div
            className="absolute inset-0 bg-coffee-primary/40 mix-blend-multiply z-10"
            aria-hidden="true"
          />
          <img
        ...
        <div className="relative z-20 text-center px-6">
          <span className="inline-block px-4 py-1.5 mb-6 rounded-full bg-coffee-primary text-coffee-accent text-xs font-bold uppercase tracking-widest">
            Freshness Delivered
          </span>
          ...
          <p className="text-white/90 max-w-xl mx-auto font-sans text-lg">
      ...
      <div className="bg-coffee-secondary/5 py-16 md:py-24 lg:py-32 px-6">
        <div className="max-w-7xl mx-auto">
          {/* Section Header */}
          <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-16 gap-8">
            <div className="max-w-2xl">
              <h2 className="text-3xl md:text-5xl font-serif text-coffee-primary mb-4">
                Choose Your Ritual
              </h2>
              <p className="text-coffee-secondary font-sans text-lg">
                Select a plan that matches your consumption. Change or cancel
                anytime without friction.
              </p>
            </div>
            <div className="hidden lg:block text-coffee-primary font-serif italic text-xl">
      ...
      <div className="bg-white py-16 md:py-24">
        <div className="max-w-5xl mx-auto px-6 grid grid-cols-1 md:grid-cols-2 gap-12 md:gap-16 items-center">
          {/* Image */}
          <div className="aspect-square bg-coffee-background overflow-hidden rounded-2xl">
            <img
          ...
          {/* Text Content */}
          <div>
            <h2 className="text-3xl font-serif text-coffee-primary mb-6">
              Roasted with Intention
            </h2>
            <p className="text-coffee-secondary font-sans mb-8 leading-relaxed">
